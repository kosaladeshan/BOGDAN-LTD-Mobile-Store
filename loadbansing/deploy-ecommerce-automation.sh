...
# =====================================================================
test_load_balancing() {
    print_section "STEP 8: Testing load balancing"
...
EOL
    ...
    print_info "Running load balancing test..."
    ~/test-load-balancing.sh
    check_success "Load balancing test completed" "Failed to run load balancing test"
    
    print_success "Load balancing tested successfully"
}
...
echo -e "\n${YELLOW}Checking load balancing:${NC}"
BACKEND1_RESPONSE=$(curl -s http://localhost:8080/health.php)
sleep 1
BACKEND2_RESPONSE=$(curl -s http://localhost:8080/health.php)

if [[ "$BACKEND1_RESPONSE" == *"server"* && "$BACKEND2_RESPONSE" == *"server"* ]]; then
    SERVER1=$(echo $BACKEND1_RESPONSE | grep -o '"server":"[^"]*"')
    SERVER2=$(echo $BACKEND2_RESPONSE | grep -o '"server":"[^"]*"')
    
    if [ "$SERVER1" != "$SERVER2" ]; then
        echo -e "${GREEN}✓ Load balancing is working correctly${NC}"
        echo -e "${GREEN}  First request served by: $SERVER1${NC}"
        echo -e "${GREEN}  Second request served by: $SERVER2${NC}"
    else
        echo -e "${RED}✗ Load balancing may not be working. Both requests served by the same server: $SERVER1${NC}"
    fi
else
    echo -e "${RED}✗ Could not verify load balancing. Server information not found in response.${NC}"
fi
